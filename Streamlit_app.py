...
n_samples = 1000
data = {
    'SensorReading': np.random.uniform(16, 84, n_samples),
    'ProductCount': np.random.uniform(7, 33, n_samples),
    'MachineStatus': np.random.choice(['On', 'Off', 'Standby'], n_samples),
    'QualityCheck': np.random.choice([True, False], n_samples, p=[0.6, 0.4])
}
...
# Use shap.bar_plot instead of force_plot for better compatibility with Streamlit
fig, ax = plt.subplots(figsize=(8, 6))
